...
Claude Code에서 여러 agent를 조율할 때의 판단 기준 — 모델 선택, 병렬화, 격리, 역할 분담.
...
같은 파일을 여러 agent가 건드리면 충돌이 난다 → 같은 파일은 같은 agent에 할당.

## worktree 격리 사용 조건

파일을 쓰는 병렬 agent는 반드시 `isolation: "worktree"` 사용.
읽기만 하는 agent (Explore, 리뷰)는 격리 불필요.
...
Agent 내부에서 commit/push 금지.
항상 `github` 스킬을 통해 외부에서 실행.
...
Developer agents ×N → 병렬 구현 (sonnet, worktree)
QA Writer agents ×N → 테스트 작성 (sonnet, parallel)
Reviewer agents ×N → Critical/Warning/Info 분류 (sonnet)
QA Runner agents ×N → 테스트 실행 → 버그 리포트 (haiku)
...
```
// 올바른 예시: 한 메시지에 두 독립 agent
Agent(prompt: "A 모듈 구현")  // 동시 실행
Agent(prompt: "B 모듈 구현")  // 동시 실행
...
- 여러 agent: 범위 불확실, 여러 코드베이스 영역 연관, 기존 패턴 파악 필요
- 최대 3개 병렬 (품질 > 수량)
