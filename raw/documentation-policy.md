...
언제, 어디에, 어떤 문서를 작성하는가. OpenAI Harness Engineering 패턴 기반.
...
- 플랜은 `active/`에서 시작 → 완료 후 `completed/`로 이동
- Completion 명령 실행 전 `active/`가 현재 상태인지 확인
...
### 스펙을 쓸 때 (docs/product-specs/)

- 사용자가 보게 될 UI/UX 정의
- 수락 기준 (이걸 충족하면 완료)
- 엣지 케이스, 오류 처리 방식

### 플랜을 쓸 때 (docs/exec-plans/)

- Tier 1 이상의 작업 (3개 이상 파일)
