...
작업 중 얻은 일반화 가능한 인사이트를 즉시 올바른 위치에 기록하는 규칙.
...
Stop 훅이 `[KNOWLEDGE CAPTURE]` 메시지를 주입하면:

1. 메시지에 명시된 skill 파일 경로를 읽는다
2. 해당 섹션 제목을 찾거나, 없으면 새 섹션을 추가한다
3. skill 파일의 기존 markdown 포맷에 맞춰 항목을 append한다
4. `github` 스킬로 커밋:
