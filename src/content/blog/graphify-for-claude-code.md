...
1. **개인 지식 창고** — 블로그, 논문, 트윗, 스크린샷을 `/raw`에 그냥 쌓는다. community detection이 나중에 묶어준다.
2. **프로젝트 LLM 컨텍스트** — 폴더별 CLAUDE.md를 직접 유지할 필요가 없다. 그래프가 구조를 대신 들고 있고, Claude는 필요할 때 `/graphify query`로 필요한 서브그래프만 가져간다.

폴더별 CLAUDE.md는 여전히 "사람 작성자의 의도" 같은 비정형 맥락엔 쓸모가 있다. 하지만 "이 폴더엔 뭐가 있고 어디서 호출되는가" 같은 구조 정보는 이제 그래프에 맡기는 게 맞다.
